...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Get waitlist entries from MongoDB
    const entries = await mongoDB.getWaitlistEntries(limit, skip)
    const stats = await mongoDB.getDatabaseStats()

    const responseTime = Date.now() - startTime
    logRequest('GET', '/api/admin/waitlist', 200, responseTime, {
      ip: clientIP,
      userAgent,
      requestId
    })
    ...
    logError('Admin waitlist fetch error', {
